...
> = {
  openrouter: {
    label: "OpenRouter",
    icon: Globe,
    desc: "Cloud gateway — 100+ models via single API key",
  },
  ollama: {
    label: "Ollama",
    icon: Horse,
    desc: "Local inference — fully air-gapped, privacy-safe",
  },
  vllm: {
    label: "vLLM",
    icon: Lightning,
    desc: "High-throughput local serving — OpenAI-compatible",
  },
  tgi: {
    label: "HuggingFace TGI",
    icon: SmileyWink,
    desc: "HuggingFace Text Generation Inference",
  },
  azure: {
    label: "Azure OpenAI",
    icon: Cloud,
    desc: "Enterprise Azure-hosted GPT models",
  },
};
...
export default function SettingsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (allowExternal !== settings?.allow_external_calls)
        updates.allow_external_calls = allowExternal;
      if (apiKey) updates.openrouter_api_key = apiKey;
      if (parsingModel !== settings?.parsing_model)
        updates.parsing_model = parsingModel;
      if (generationModel !== settings?.generation_model)
        updates.generation_model = generationModel;

      // Azure fields
      if (azureEndpoint) updates.azure_endpoint = azureEndpoint;
      if (azureApiKey) updates.azure_api_key = azureApiKey;
      if (azureDeployParsing)
        updates.azure_deployment_parsing = azureDeployParsing;
      if (azureDeployGeneration)
        updates.azure_deployment_generation = azureDeployGeneration;
    ...
    <div className={styles.page}>
      <div className={styles.container}>
        ...
        <p className={styles.subtitle}>
          Configure your MindFlayer instance — providers, models, and security
        </p>

        {/* Backend Status */}
        <div className={`glass-card ${styles.card}`}>
          <h2 className={styles.cardTitle}>
            <Gear size={24} weight="duotone" /> Backend Status
          </h2>
          <div className={styles.statusRow}>
            ...
            <span>
              ...
              {backendStatus === "offline" &&
                "Backend is offline — start it with: cd backend && uv run uvicorn main:app --reload"}
            </span>
          </div>
          {settings && (
            <div className={styles.statusInfo}>
              ...
              <span
                className={`badge ${settings.has_api_key ? "badge-green" : "badge-red"}`}
              >
                API Key: {settings.has_api_key ? "Configured" : "Not Set"}
              </span>
              <span className="badge badge-purple">
                Provider:{" "}
                {PROVIDER_META[settings.llm_provider]?.label ||
        ...
        <div className={`glass-card ${styles.card}`}>
          <h2 className={styles.cardTitle}>
            <LinkSimple size={24} weight="duotone" /> LLM Provider
          </h2>
          <p className={styles.cardDesc}>
            Choose your LLM backend. Local providers (Ollama, vLLM, TGI) keep
            all data on-premise.
          </p>
                ...
                >
                  <div className={styles.providerHeader}>
                    <span className={styles.providerIcon}>
                      {<meta.icon size={24} weight="duotone" />}
                    </span>
                    <span className={styles.providerName}>{meta.label}</span>
                    {status && (
                      <span
                        className={`${styles.providerStatus} ${status.available ? styles.providerOnline : styles.providerOffline}`}
                      >
                        {isBlocked ? (
                  ...
                  {status?.is_local && (
                    <span className={styles.localBadge}>
                      <Key size={12} weight="duotone" /> Local
                    </span>
                  )}
                </button>
              );
        ...
        <div className={`glass-card ${styles.card}`}>
          <h2 className={styles.cardTitle}>
            <ShieldCheck size={24} weight="duotone" /> Data Privacy
          </h2>
          <div className={styles.toggleRow}>
            <div>
              <label className={styles.fieldLabel}>Local-Only Mode</label>
              <p className={styles.fieldHint}>
                When enabled, blocks all external API calls (OpenRouter, Azure).
                Only local providers are allowed.
              </p>
            </div>
            <button
          ...
          {!allowExternal && (
            <div className={styles.privacyWarning}>
              <WarningCircle size={18} weight="duotone" /> Local-only mode is
              active. Cloud providers (OpenRouter, Azure) are blocked. Use
              Ollama, vLLM, or TGI.
            </div>
          )}
        </div>
        ...
        {provider === "openrouter" && (
          <div className={`glass-card ${styles.card}`}>
            <h2 className={styles.cardTitle}>
              <Globe size={24} weight="duotone" /> OpenRouter Configuration
            </h2>
            <p className={styles.cardDesc}>
        ...
        {provider === "azure" && (
          <div className={`glass-card ${styles.card}`}>
            <h2 className={styles.cardTitle}>
              <Cloud size={24} weight="duotone" /> Azure OpenAI Configuration
            </h2>
        ...
        <div className={`glass-card ${styles.card}`}>
          <h2 className={styles.cardTitle}>
            <Cpu size={24} weight="duotone" /> AI Models
          </h2>

          <div className={styles.fieldGroup}>
            <label className={styles.fieldLabel}>Parsing Model</label>
            <p className={styles.fieldHint}>
              Used for converting natural language requirements to structured
              format
            </p>
            <input
              className="input"
          ...
          <div className={styles.fieldGroup}>
            ...
            <p className={styles.fieldHint}>
              Used for generating intelligent test code (pick a strong code
              model)
            </p>
            <input
              className="input"
          ...
          {(() => {
            ...
            if (
              ...
              return (
                <div className={styles.modelSuggestions}>
                  <span className={styles.suggestLabel}>
                    Available on {PROVIDER_META[provider]?.label}:
                  </span>
                  <div className={styles.suggestList}>
        ...
        <div className={styles.actions}>
          ...
          {saved && (
            <div className={styles.success}>
              <CheckCircle size={16} weight="bold" /> Settings saved
              successfully!
            </div>
          )}
          <button
          ...
          >
            {saving ? (
              ...
              <>
                <FloppyDisk size={20} weight="duotone" /> Save Settings
              </>
            )}
          </button>
        </div>
