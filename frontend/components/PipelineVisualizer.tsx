    ...
    <div className={styles.pipeline}>
        ...
        {STAGES.map((stage, i) => {
          ...
          return (
            <div key={stage.key} className={styles.stageWrapper}>
              {i > 0 && (
                <div
                  className={`${styles.connector} ${status !== "pending" ? styles.connectorActive : ""}`}
                />
              )}
              <div className={`${styles.stage} ${styles[status]}`}>
                <div
                  className={`${styles.stageIcon} ${status === "active" ? styles.activeIcon : ""}`}
                >
                  {status === "active" ? (
                    <>
                ...
                <div className={styles.stageInfo}>
                  ...
                  {stageMessages[stage.key] && (
                    <span className={styles.stageMsg}>
                      {stageMessages[stage.key]}
                    </span>
                  )}
                </div>
      ...
      {currentStage === "complete" && (
        <div className={styles.completeMsg}>
          <Confetti size={20} weight="duotone" /> All stages complete!
        </div>
      )}
      {currentStage === "error" && (
        <div className={styles.errorMsg}>
          <XCircle size={20} weight="duotone" /> Pipeline failed — check the
          error below.
        </div>
      )}
    </div>
